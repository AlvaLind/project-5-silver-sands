...
class OrderForm(forms.ModelForm):
    ...
    def clean_full_name(self):
        ...
        if len(full_name) > 40:
            raise ValidationError(
                'Full Name must be no longer than 40 characters.')
    ...
    def clean_postcode(self):
        ...
        if len(postcode) < 4 or len(postcode) > 10:
            raise ValidationError(
                'Postal code must be between 4 and 10 digits long.')
        return postcode
    ...
    def clean_street_address1(self):
        ...
        if len(street_address1) > 50:
            raise ValidationError(
                'Street Address 1 must be no longer than 50 characters.')
        if not re.match(r'^[a-zA-Z0-9\s,.-/&]*$', street_address1):
            raise ValidationError(
                'Street Address 1 can only contain '
                'letters, numbers, spaces, and limited punctuation.')
    ...
    def clean_street_address2(self):
        ...
        if street_address2:
            if street_address2 and len(street_address2) > 50:
                raise ValidationError(
                    'Street Address 2 must be no longer than 50 characters.')
            if not re.match(r'^[a-zA-Z0-9\s,.-/&]*$', street_address2):
                raise ValidationError(
                    'Street Address 2 can only contain letters, numbers, '
                    'spaces, and limited punctuation.')
    ...
    def clean_county(self):
        ...
        if county and len(county) > 40:
            raise ValidationError(
                'County, State or Locality must be '
                'no longer than 40 characters.')
        if not re.match(r'^[a-zA-Z\s]+$', county):
            raise ValidationError(
                'County or State must contain only letters and spaces.')
        return county
    ...
    def clean_town_or_city(self):
        ...
        if town_or_city and len(town_or_city) > 40:
            raise ValidationError(
                'Town or City must be no longer than 40 characters.')
        if not re.match(r'^[a-zA-Z\s]+$', town_or_city):
            raise ValidationError(
                'Town or City must contain only letters and spaces.')
        return town_or_city

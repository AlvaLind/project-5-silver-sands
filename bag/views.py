...
def add_to_bag(request, item_id):
    ...
    if wine.stock == 0 or not wine.available:
        messages.error(
            request,
            f'Sorry, {wine.name} is currently out of stock and unavailable.')
    ...
    if current_bag_quantity + quantity > wine.stock:
        messages.error(
            request,
            f'Sorry, there are only {wine.stock} {wine.name} left in stock.')
        return redirect(redirect_url)
    ...
    if item_id in list(bag.keys()):
        ...
        messages.success(
            request,
            f'Updated {wine.name} quantity to {bag[item_id]}')
    else:
        bag[item_id] = quantity
        messages.success(
            request,
            f'Successfully added {wine.name} to your bag')
